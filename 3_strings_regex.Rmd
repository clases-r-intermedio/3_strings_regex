...
Probablemente en su experiencia programando, con `R` u otro *software*, han .pur[creado] o .pur[manipulado] strings (o cadenas de caracteres) en en alguna ocasión.
...
4. Crea dos nuevas variables en tu dataframe: 
    - **"first_letters"**, que contenga las s primeras letra de cada nombre y 
...
```{r}
guaguas <- guaguas %>% 
  mutate(first_letters = str_sub(nombre, 1, 2),
         last_letters = str_sub(nombre, -2, -1))
guaguas %>% select(nombre, first_letters, last_letters) %>% head()
```
...
Mi correo electrónico del trabajo es <_ifaglonij@ine.gob.cl_> 
...
La url es la siguiente: https://gitlab.com/inesscc/demostracion_git.git
